...
export const SeoPageImportSchema = z.object({
  slug: z.string().min(2),
...
export const ComparisonImportSchema = z.object({
  ...
  whoBuysHere: z.object({
    area1Profile: z.string().default(""),
    area2Profile: z.string().default("")
  }).optional()
});

// --- AreaComparison ---
export const VerdictForBudgetSchema = z.object({
  budgetLabel: z.string(),
  recommendedArea: z.enum(["area1", "area2"]),
  reason: z.string()
});
...
export const WhoBuysHereSchema = z.object({
  area1Profile: z.string(),
  area2Profile: z.string()
});

const PopulatedAreaSchema = z.record(z.unknown());

export const AreaComparisonSchema = z.object({
  _id: z.string().optional(),
  slug: z.string(),
  area1: z.union([z.string(), PopulatedAreaSchema]),
  area2: z.union([z.string(), PopulatedAreaSchema]),
  isAutoGenerated: z.boolean().default(false),
  isActive: z.boolean().default(true),
  sortOrder: z.number().default(0),

  // SEO (admin can override)
  metaTitle: z.string().optional(),
  metaDescription: z.string().optional(),
  heroHeading: z.string().optional(),
  introText: z.string().optional(),
  ...
  // Editorial
  verdict: z.string().optional(),
  verdictForBudgets: z.array(VerdictForBudgetSchema).default([]),
  keyDifferences: z.array(KeyDifferenceSchema).default([]),
  whoBuysHere: WhoBuysHereSchema.optional(),
