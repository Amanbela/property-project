...
export async function POST(request: Request) {
  try {
    ...
    const { budgetMin, budgetMax, purpose, propertyType, lifestyle, preferredLocation } = body;

    if (!budgetMax || !purpose || !propertyType || !lifestyle) {
      return NextResponse.json(
        { error: "Missing required fields: budgetMax, purpose, propertyType, lifestyle" },
        { status: 400 }
      );
    }
    ...
    const scoredAreas = await Promise.all(
      areas.map(async (area) => {
        // Price per sqft (check averagePricePerSqft or averagePrice)
        ...
        // Get traffic rating: area.trafficScore or normal trafficCondition
        let trafficScore = 60;
        ...
        // Schools & Hospitals suitability scores based on real database entries length
        ...
        return {
          ...
          whyRecommended: whyRecommended.slice(0, 4), // Top 4 reasons
        };
      })
    );

    // Filter out completely low matching scores unless all are low, and sort
    const rankedAreas = scoredAreas
      .sort((a, b) => b.matchScore - a.matchScore)
      .slice(0, 5); // Return top 3-5 areas

    return NextResponse.json(rankedAreas);
  } catch (error: unknown) {
    const err = error as Error;
    console.error("Error in /api/recommendations:", err);
    return NextResponse.json({ error: err.message || "Failed to calculate recommendations" }, { status: 500 });
  }
}
